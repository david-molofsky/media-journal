...
/** Normalises a detected barcode to a 12-digit UPC-A string. Some
 * BarcodeDetector implementations report a UPC-A code under format
 * "upc_a" directly; others report it as a 13-digit "ean_13" value with
 * a leading zero (UPC-A is a strict subset of EAN-13). Anything else
 * is ignored — this dialog only opens from the Film form, so no ISBN-
 * style prefix filtering is needed, but a barcode of the wrong length
 * shouldn't be sent to the lookup as if it were a valid UPC. */
function toUpc12(barcode: { rawValue: string; format: string }): string | null {
